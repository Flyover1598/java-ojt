...
- CRUDに**ルーズに**対応している。
    ...
    - PUT/PATCH: Update
        - PUTは全体の塗りつぶしを行う。
            - ターゲットリソースが現存せず、これを新しく作成した場合、201を返さなければならない（RFC 9110）。
            - ターゲットリソースが存在し、書換えに成功した場合、200（又は204）を返さなければならない（同上）。
        - PATCHは一部の書換えを行う。
            - ターゲットリソースが存在しない場合、patchの内容または権限の関係上作成が不可能の場合は、404を返してよい（RFC 5789）。
            - ターゲットリソースにおいて存在を予想される構造が実際にはなかった場合は、409を返してよい（同上）。
    - DELETE: Delete
### OpenAPI
